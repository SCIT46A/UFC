...
public class RewardDeliveryService {
    ...
    public Map<String, Object> getRewardDeliveryData(List<Long> campaignIds) {
        ...
        List<Map<String, Object>> deliveryDTOs = deliveries.stream().map(delivery -> {
            ...
            donationDto.put("donationId", Optional.ofNullable(delivery.getDonation().getDonationId()).orElse(0L));
            donationDto.put("campaignTitle",
                    Optional.ofNullable(delivery.getDonation().getCampaign().getTitle()).orElse("-"));
            donationDto.put("userName",
                    Optional.ofNullable(delivery.getDonation().getUser().getUserName()).orElse("미등록"));
            donationDto.put("donorPhone",
                    Optional.ofNullable(delivery.getDonation().getUser().getPhoneNumber()).orElse("-"));
            donationDto.put("donorAddress",
                    Optional.ofNullable(delivery.getDonation().getUser().getUserAddress()).orElse("-"));
            donationDto.put("dueDate",
                    Optional.ofNullable(delivery.getDonation().getCampaign().getSendDate()).orElse(null));
        ...
        Map<String, Long> deliveryCounts = deliveries.stream()
                .collect(Collectors.groupingBy(delivery -> Optional.ofNullable(delivery.getStatus()).orElse("미등록"),
                        Collectors.counting()));
    ...
    public void updateInvoices(List<InvoiceUpdateRequest> updateRequests) {
        for (InvoiceUpdateRequest request : updateRequests) {
            RewardDeliveryEntity delivery = rewardDeliveryRepository.findByDonation_DonationId(request.getDonationId())
                    .orElseThrow(() -> new IllegalArgumentException(
                            "기부번호에 해당하는 배송 정보를 찾을 수 없음: " + request.getDonationId()));
    ...
    public void updateInvoice(Long rewardDeliveryId, String courier, String trackingNumber) {
        RewardDeliveryEntity delivery = rewardDeliveryRepository.findById(rewardDeliveryId)
                .orElseThrow(() -> new IllegalArgumentException("🚨 배송 정보를 찾을 수 없음: " + rewardDeliveryId));
